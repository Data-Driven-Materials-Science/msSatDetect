...
def trainModel(objectName, json_path_train, json_path_val, maxIterations=5000, checkpointIteration=5001, experimentNum='0', 
    outputDir=None, modelDevice='cuda', detectionsPerImage=250, visualizeTrainVal=False, 
    imagesPerBatch=1,numClasses=1, inputMaskType='polygon'):
    '''
    Parameters:
    -----------
        ...
        outputDir: Documentation Purposes. Defines a couple director and file names
            Type: pathLib Path()
            Default Value: '../models/' + objectName + '_output_' + experimentNum + '/'
            Note on changing value: 
                This will be the overarching folder that the model will saved to. If you wish to go to a file different than the default,
                pass in a file path. However, be aware of writing over previous models. 
        modelDevice:
            Type: String
            Default Value: 'cuda'
    ...
    Returns
    ------------
        outputDir: location of saved model(s)
            Type: pathlib Path()
    
    '''
    if outputDir == None:
        outputDir = Path('../models/' + objectName + '_output_' + experimentNum + '/')
    ...
    for d in [dataset_train, dataset_valid]:
        MetadataCatalog.get(d).set(**{'thing_classes': [objectName]})
    ...
    cfg.INPUT.MASK_FORMAT = inputMaskType                   # masks generated in VGG image annotator are polygons
    ...
    cfg.SOLVER.IMS_PER_BATCH = imagesPerBatch               # number of images per batch (across all machines)
    ...
    cfg.MODEL.ROI_HEADS.NUM_CLASSES = numClasses            # Since we are training separate models for particles and satellites there is only one class output
    cfg.TEST.DETECTIONS_PER_IMAGE = detectionsPerImage      # maximum number of instances that can be detected in an image (this is fixed in mask r-cnn)
    ...
    print('Downloading Weights from source: {}'.format(weights_path))
